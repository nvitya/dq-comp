...
string OModuleIntf::DqmIfBuildOptions() const
{
  ...
  if (OPTLEVEL_O1 == g_opt.optlevel) result += ";inline=explicit";
                                      ...
                                      bool interface_only)
{
  ...
  if (g_opt.no_use_sys || OModulePath::SuppressesImplicitSys(module_path, module_root_dir))
  {
...
bool OModuleIntf::ApplyDqmIfAttributes(OValSym * avalsym, const SDqmIfAttributes & attrs)
{
  ...
  avalsym->attr_is_used          = (attrs.flags & (1ull << 19));
...
bool OModuleIntf::ReadInlineValue(ODqmIfReader & reader, OType * atype, OValue *& rvalue)
{
  ...
  else if (TK_FUNCREF == rtype->kind)
  {
    ...
    if (value != 0)
    {
      return reader.Fail("Only nil function reference values are supported in DQM interface loading");
    }
